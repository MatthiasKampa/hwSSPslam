...
Usage:
  python3 -m runners.datasets selftest              # bit-exact vs run_log
  python3 -m runners.datasets run belg              # shipped config on one name
  python3 -m runners.datasets run stata lean        # named config
"""
import sys
import time
...
def load(name, cap=None):
    ...
    d = DATASETS[name]
    if d["kind"] == "spot":
        import runners.spot as ssp_spot
...
def evaluate(bundle, fin):
    ...
    elif bundle["eval"] == "exact":              # synth bench / spot
        gt = bundle["gt"]
        ok = bundle.get("gt_ok")
...
def ref_for_keys(bundle):
    ...
    if bundle["eval"] == "exact":                # synth bench / spot
        ok = bundle.get("gt_ok")
        m = np.ones(n, np.uint8) if ok is None else ok.astype(np.uint8)
        return bundle["gt"][:, :2].astype(np.float32), m
    if bundle["eval"] == "gfs":
