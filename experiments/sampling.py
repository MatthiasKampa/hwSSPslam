...
Usage:
  python3 -m experiments.sampling selftest
  python3 -m experiments.sampling interp fr079           # stage 1 sweep
  python3 -m experiments.sampling segint fr079           # stage 2: integral modes
  python3 -m experiments.sampling renorm fr079           # stage 3: alpha sweep
"""
import sys
...
import sspslam.bounded as BB      # alias for constants inside copied bodies
import sspslam.quantized as F          # (the verbatim body assigns a local `B`)
...
def stage_spot(worlds=("mixed", "corridor"), seeds=(11,)):
    ...
    import runners.synth as ssp_synth
    for world in worlds:
        for seed in seeds:
