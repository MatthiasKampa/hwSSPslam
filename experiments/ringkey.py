...
Motivation (RESULTS.md R3/R4 + baselines/scancontext.py): the MIT Infinite-Corridor
...
place-recognition information there. baselines/scancontext.py then proved the info is
...
Design (subclass; NO edit to any shipped module — same pattern as experiments/angles.py
and baselines/scancontext.py):
...
Usage:
  python3 -m experiments.ringkey selftest
  python3 -m experiments.ringkey mit [data/mit.log]     # retrieval + ATE, both systems
  python3 -m experiments.ringkey intel [data/intel.log] # sanity: ring-key must not regress
"""
...
class RingKeySLAM(H.HybridSLAM):
    ...
    def _try_drought(self, pts, w):
        ...
        #      HybridSLAM._try_drought (experiments/hier.py ~1394-1476) ---------------
        _, rel = self.kf_ref[k]
