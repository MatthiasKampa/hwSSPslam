...
simulates exactly that sensor class over the validated `worlds.py`
...
runner works unchanged on synthetic data:
    ...
    import ssp_synth, ssp_datasets as DS
...
Usage:
  python3 ssp_synth.py bench            # shipped vs E2 on 3 worlds x 2 seeds
  python3 ssp_synth.py fov              # 180 vs 260 vs 360 FOV, same worlds
"""
import sys
...
def bench(worlds=("mixed", "office", "corridor"), seeds=(11, 12)):
    ...
    for world in worlds:
        for seed in seeds:
            ...
            odo = b["odom"]
            e = np.linalg.norm(
                __import__("ssp_slam_carmen").align_se2(
