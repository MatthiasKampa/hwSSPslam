...
earlier experiment's file. Shipped core is `sspslam/bounded.py`,
`sspslam/encoder.py`, `sspslam/lattice.py`, `sspslam/frontend.py`, and the
driver `runners/carmen.py`. The rest of `sspslam/` (`quantized.py`,
`lattice_presets.py`, `worlds*.py`, `bench.py`) is load-bearing infrastructure
under the same subclass/import-only discipline.

- New backend behaviour → subclass `BoundedSLAM` and override the method
  (e.g. `experiments/aniso.py: AnisoBoundedSLAM(AnisoMixin, B.BoundedSLAM)`).
- Reusing machinery → `import` it (e.g. `experiments/hybrid.py` imports
  `experiments.flow.FlowField`; `experiments/multisession_verify.py` imports the
  PCM engine).
- Sweeping a shipped **constant** (e.g. the lattice) → patch the module global at
  runtime in a `scratch/` harness, rebuild everything derived from it, and never
  touch the source. Consolidated pattern: `sspslam/lattice_presets.py`.
...
- If GT is used to establish a **diagnostic upper bound** (e.g. "what would a
  ...
  deployable**. Examples done right: `experiments/mit_gtverify.py`, `ssp_hybrid.run_multi`
...
printing numbers. (The shipped `runners/carmen.py` writes one Agg PNG by
...
- **Scratch**: throwaway code/logs live in `scratch/` (gitignored wholesale),
  named `scratch_<topic>.{py,log,out}`. Run them from the repo root with
  `PYTHONPATH=. python3 scratch/scratch_<topic>.py`. Verify nothing un-ignored
  would be swept before any `git add`. (Pre-2026-07-13 scratch sat at the repo
  root; it was moved into `scratch/` unmodified, so old files still import the
  historical flat module names — evidence, not runnable code.)
