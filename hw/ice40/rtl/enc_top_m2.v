...
module enc_top_m2 #(
    ...
    always @(posedge clk) begin
        ...
        if (rxv) begin
            ...
            4'd12: begin
                m_sh <= rxd[3:0];
                if (bhdr) bpsh <= 2'd2;    // push 2 FIFO words (below);
                else req_m <= 1'b1;        // pusher advances prx
                if (!bhdr) prx <= 0;
            end
            4'd13: begin bmode <= rxd[0]; prx <= 14; end
    ...
    localparam E_IDLE = 0, E_POP = 1, E_GO = 2, E_RUN = 3, E_CLR = 4,
               ...
               E_BP1 = 10, E_BP2 = 11, E_BGO = 12, E_BWT = 13,
               E_BT = 14, E_BTS = 15;
    reg [3:0]  est = E_IDLE;
    ...
    always @(posedge clk) begin
        ...
        E_BT: begin                     // totals: sum the 4 ring Re parts
            ...
            s_sel <= s_sel + 1;
            tsel  <= tsel + 1;
            if (tsel == 2'd3) begin rb_b <= 0; est <= E_BTS; end
        end
