...
//
// ---- v8 (solo_tracker2, ECP5 track; golden hw/ecp5/host/solo2.py) ----
// Derived from the gated v7 solo_tracker.v (ice40, UNTOUCHED) with ONE
// addition: the `novel` output — nearest-anchor d2 > NOV_D2 at the
// prediction, latched by the nearest scan every step. top_solo2 gates
// OPENING a new segment on it (novelty fold: map budget buys AREA).
// Tracking semantics are BIT-IDENTICAL to v7 (the l1/lr lean fixtures
// gate this file verbatim). The 2026-07-16 hunter decomposition bench
// REFUTED (crispness, do not re-add without new data): +-9 deg fine
// window 10.73 -> 10.12 pts/cell, rho edge retry (same), staged/global
// lost re-search (never fired / no gain), heading clamp 8.2, abs
// commit floor 7.66. Novelty ALONE: stock 9.29 -> 10.73 (odom 7.27).
`timescale 1ns/1ps
module solo_tracker2 #(
    parameter NSEG = 64,
    parameter REFINE = 1,
    parameter NOV_D2 = 670761          // (0.8 m / U)^2
) (
    input  wire        clk,
    // anchor table load
    input  wire        anc_we,
    ...
    output reg         novel = 1,        // pred > NOV_D2 from all anchors
    ...
    always @(posedge clk) begin
        ...
        T_AR0: begin
            anc_ra <= {ai, 3'd0}; nph <= 0;
            novel <= (best_d2 > NOV_D2);         // v8 novelty bit
            tst <= T_AR1;
        end
        T_AR1: begin
            anc_ra <= anc_ra + 1;
