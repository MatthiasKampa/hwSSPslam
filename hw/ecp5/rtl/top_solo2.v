// top_solo2.v — v8 STANDALONE SLAM top (ECP5 track). Derived from the
// gated v7 top_solo.v (ice40, UNTOUCHED) with ONE functional change:
// NOVELTY-GATED FOLDING — a new segment opens only when the tracker's
// nearest-anchor scan flags the prediction as novel (> NOV_D2 from
// every anchor), so the resident map's budget buys AREA, not time
// (hunter bench 2026-07-16: tracking 1937 -> 2467+ of 2810 kf, map
// crispness 9.29 -> 10.45 pts/cell). An OPEN segment always finishes
// its SEG_KF folds. Tracker = solo_tracker2 (v8 features + `novel`).
// Golden contract: hw/ecp5/host/solo2.py; gates: solo2_gates.py.
//
...
module top_solo2 #(
    ...
    parameter NSEG = 64,
    parameter SEG_KF = 5,
    parameter NOV_D2 = 670761,     // (0.8 m / U)^2 novelty radius
    parameter REFINE = 0
) (
    input  wire clk,
    input  wire RX,
    ...
    wire trk_novel;
    solo_tracker2 #(.NSEG(NSEG), .REFINE(REFINE), .NOV_D2(NOV_D2))
             trk (.clk(clk),
                 .novel(trk_novel),
    ...
    always @(posedge clk) begin
        ...
        K_FOLDQ: begin
            if (mapping_en && npts_total >= 16'd5
                && n_seg < NSEG[6:0]
                // v8 novelty gate: OPENING a new segment needs a novel
                // prediction (boot n_seg==0 exempt); an open segment
                // always completes its SEG_KF folds
                && (seg_open || n_seg == 7'd0 || trk_novel)) begin
                if (!seg_open) begin
